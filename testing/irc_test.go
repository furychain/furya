...
func (suite *IrcTestSuite) SetupConnection(rollappChain *ibctesting.TestChain, hubChain *ibctesting.TestChain) *ibctesting.Path {
	...
	// create the client of furya on the rollapp chain
	err = rollappEndpoint.CreateClient()
	suite.Require().Nil(err)
	// update rollapp state
	err = suite.UpdateRollappState(rollappEndpoint.Chain.ChainID, bds)
	// create the client of the rollapp on furya
	err = furyaEndpoint.CreateClient()
	suite.Require().Nil(err)
	...
	err = furyaEndpoint.ConnOpenConfirm()
	suite.Require().Nil(err)
	...
	furyaEndpoint.ChannelConfig.PortID = ibctransfertypes.PortID
	furyaEndpoint.ChannelConfig.Version = ibctransfertypes.Version
	...
	err = suite.UpdateRollappState(rollappEndpoint.Chain.ChainID, bds)
	// send confirmation to the hub
	err = furyaEndpoint.ChanOpenConfirm()
	suite.Require().Nil(err)
...
// TestSuite initialize furyint rollapp and furya hub chain
