...
func SetupFuryenstionTestingApp() (ibctesting.TestingApp, map[string]json.RawMessage) {
	...
	app := furyaapp.Setup(checkTx)
	return app, furyaapp.NewDefaultGenesisState(encCdc.Marshaler)
}

// SetupTestingApp is a callback for the ibctesting to create a chain
// the test creates a furya chain and furyint chain
...
func (suite *IrcTestSuite) FinalizeState() (err error) {
	disputePeriodInBlocks := suite.hubChain.App.(*furyaapp.App).RollappKeeper.DisputePeriodInBlocks(suite.hubChain.GetContext())
